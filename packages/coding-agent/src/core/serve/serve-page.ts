...
<aside class="rail"><div class="pi-watermark" aria-hidden="true">π</div><div class="rail-heading"><span>Sessions</span><button id="show-connection-form" type="button" title="Connect another Pi session" aria-label="Connect another Pi session">+</button></div><section id="sessions" class="rail-panel"><form id="connection-form" class="hidden"><label>Pi control URL<input id="connection-url" type="url" placeholder="http://127.0.0.1:4173/?token=…" required></label><button type="submit">Connect</button></form><div id="connection-list"></div></section></aside>
