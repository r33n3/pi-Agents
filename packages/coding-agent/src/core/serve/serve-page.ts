...
import { roomActivitySummary, taskActivitySummary } from "./activity-summary.ts";
...
import { WORKSPACE_STARTUP_HTML, WORKSPACE_STARTUP_STYLE } from "./workspace-startup-page.ts";
...
async function serveAgentTasks(
	...
	if (request.method === "GET" && suffix === "") {
		...
		json(response, 200, {
			tasks: url.searchParams.get("view") === "activity" ? tasks.map(taskActivitySummary) : tasks,
		});
		return;
	}
	if (request.method === "GET" && suffix) {
...
async function serveAgentRooms(
	...
	try {
		...
		if (request.method === "GET" && suffix === "") {
			json(response, 200, {
				rooms: service.listDefinitions(),
				runs:
					url.searchParams.get("view") === "activity"
						? service.listRuns().map(roomActivitySummary)
						: service.listRuns(),
		...
		if (request.method === "POST" && suffix.endsWith("/message")) {
			const body = object(await readJsonBody(request), "team message request");
			json(
				response,
				202,
				await service.message(suffix.slice(0, -"/message".length), requiredString(body.goal, "goal")),
			);
			return;
		}
		if (request.method === "PUT" && suffix) {
		...
		if (request.method === "GET" && suffix) {
			...
			json(
				...
				definition ? 200 : 404,
				definition
					? { room: definition, runs: service.listRuns(suffix), memory: service.memory(suffix) }
...
${WORKSPACE_STARTUP_STYLE}
</style></head><body data-workspace-loading="loading">
${WORKSPACE_STARTUP_HTML}
