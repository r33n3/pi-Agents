...
import { runBrowserCommand } from "./cli/browser-command.ts";
...
import { ServeHost } from "./core/serve/serve-host.ts";
import {
...
export async function main(args: string[], options?: MainOptions) {
	...
	if (await runBrowserCommand(args)) {
		return;
	}
	...
	let serveHost: ServeHost | undefined;
	if (parsed.serve) {
		if (appMode !== "interactive") {
		...
		const result = await serveHost.start();
		if (result.port !== requestedPort) {
			console.error(`Port ${requestedPort} was in use; Pi selected ${result.port}`);
		}
		if (host !== "127.0.0.1" && host !== "::1" && host !== "localhost") {
			console.error(chalk.yellow(`Warning: --serve is binding outside localhost at ${host}:${result.port}`));
		}
		console.error(`Pi web control: ${result.url}`);
